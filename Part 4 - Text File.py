...
def process_of_result(Pass,Defer,Fail): #check all requirement,give final output based on that
    if Pass == 120:
        print("progress")
        progress.append(0)
        values = [Pass, Defer, Fail]
        progress_from_user.append(values)
        ...
        trailer.append(0)
        values = [Pass, Defer, Fail]
        trailer_from_user.append(values)
        ...
        retriever.append(0)
        values = [Pass, Defer, Fail]
        retriever_from_user.append(values)
        ...
        exclude.append(0)
        values = [Pass, Defer, Fail]
        exclude_from_user.append(values)
...
def validationTotal():   ##Get sum of pass, defer, fail and check sum is below or equals to 120
    ...
    while True:
        ...
        if Total != 120:
            ...
            process_of_result(Pass,Defer,Fail)  # checkout the total credit mark given by user
            break
...
def staff_histogram():  # printing the horizontal histogram  base on that
    ...
    print("=" * 80)  #according to the user input credit level printing the horizontal  histrogram  with "=" mark
...
def get_data_from_list():   #get the data from histogram and printed as a list
    print("=" * 80)
    for i in progress_from_user:
        print("Progress-",",".join(str(x) for x in i))
    for i in trailer_from_user:
        print("Progress (module trailer)-",",".join(str(x) for x in i))
    for i in retriever_from_user:
        print("Module retriever - ",",".join(str(x) for x in i))
    for i in exclude_from_user:
...
def convert_to_text_file():  ##take the data from list and print it to a text file
    global  fo
    fo =open("coursework.txt","w")
    for i in progress_from_user:
        fo.write("Progress-"+",".join(str(x) for x in i) + "\n")
    for i in trailer_from_user:
        fo.write("Progress (module trailer)-"+",".join(str(x) for x in i) + "\n")
    for i in retriever_from_user:
        fo.write("Module retriever - "+",".join(str(x) for x in i) + "\n")
    for i in exclude_from_user:
        fo.write("Exclude-"+",".join(str(x) for x in i) + "\n")
    fo.close()

def read_text_file():  #take the data from text file and printed it in to  
    global fo
    fo = open("coursework.txt", "r")
    with fo:
        count = 1
        for line in fo:
            print(line,end="")
...
# reference-- https://www.w3schools.com/python/ ----https://www.geeksforgeeks.org/python-programming-language/
